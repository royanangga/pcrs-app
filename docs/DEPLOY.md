...
4. Buka folder `supabase/migrations/` di paket file ini — jalankan **setiap file .sql di dalamnya satu per satu, urut sesuai nama file** (nama file diawali angka tanggal, jadi otomatis sudah berurutan). Tiap file: copy semua isinya → paste ke SQL Editor → **Run** → pastikan muncul "Success" → lanjut ke file berikutnya.
   - Ini menjalankan seluruh riwayat skema database dari awal sampai versi terbaru (termasuk fitur Invoice). Detail tiap file ada di `docs/DATABASE_MIGRATIONS.md`.
   - Kalau project ini **sudah pernah jalan sebelumnya** dan kamu cuma mau update ke versi terbaru, cukup jalankan file migrasi yang **belum pernah dijalankan** (lihat `docs/DATABASE_MIGRATIONS.md` untuk cara cek "sudah sampai mana").
5. Di menu kiri klik **Edge Functions** → **Deploy a new function** → beri nama `admin-user-ops` → copy-paste isi `supabase/functions/admin-user-ops/index.ts` → Deploy. (Fungsi ini dipakai Admin Panel untuk membuat/menonaktifkan akun user.)
6. Di menu kiri klik **Project Settings** (ikon gear) → **API**.
