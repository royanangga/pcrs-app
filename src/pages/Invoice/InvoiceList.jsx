...
export default function InvoiceList({ profile }) {
  ...
  async function handleImportFile(e) {
    ...
    try {
      ...
      for (const ws of wb.worksheets) {
        ...
        try {
          ...
          const { data: inv, error: invErr } = await supabase.from('invoice_invoices').insert({
            ...
            attn, currency: 'IDR', batch: '', remark, status: 'Diajukan', approval_status: 'approved',
            created_by: profile.id, approved_by: profile.id, approved_at: new Date().toISOString(),
          }).select().single()
      ...
      setImportMsg(`Selesai: ${imported} invoice berhasil diimpor, ${skipped} dilewati.${errors.length ? ' Error: ' + errors.slice(0, 3).join('; ') : ''}`)
      load()
    } catch (err) {
  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {selected.size > 0 && (
            <button className="btn btn-sm btn-neutral" onClick={doBatchPrint}>
              <Icon name="printer" size={13} /> Print {selected.size} Invoice
            </button>
          )}
      ...
      {loading ? (
        ...
        <div className="table-scroll">
          <table>
            <thead>
              <tr>
                <th></th><th>No. Invoice</th><th>Tanggal</th><th>Customer</th><th>Total</th><th>Status</th><th></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((r) => {
